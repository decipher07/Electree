...
for page_num in range(2, 43):
    ...
    for i in range(col):
        ...
        for j in range(1):
            ...
            coords = str(x0) + ',' + str(y0) + ',' + str(x1) + ',' \
                + str(y1)
            parsed_text = pdf.pq('LTTextLineHorizontal:in_bbox("%s")'
                                 % coords).text()
            parsed_text = re.sub(' +', ' ', parsed_text)
            print (parsed_text)
            uni_parsed_text = unidecode(str(parsed_text))
            uni_parsed_text = re.sub(' +', ' ', uni_parsed_text)

            # print unidecode(str(parsed_text)),"\n"
            ...
            if 'mlpg' in uni_parsed_text and 'aadk' in uni_parsed_text:
                ...
                try:
                    name = temp_li[temp_li_uni.index('npm')
                        + 2:temp_li_uni.index('vptp')]
                ...
                if 'pnt' in uni_parsed_text:
                    ...
                    husband_name = temp_li[temp_li_uni.index('npm')
                        + 3:temp_li_uni.index('mkpn') + 1]
                    ...
                    father_name = temp_li[temp_li_uni.index('npm')
                        + 3:temp_li_uni.index('mkpn') + 1]
                ...
                house_num = temp_li_uni[temp_li_uni.index('spkhdp')
                    + 2:temp_li_uni.index('spkhdp') + 3]
                house_num = ''.join(house_num)
                dic['house_num'] = house_num.strip()

                # print dic

                sex = temp_li[temp_li_uni.index('mlpg')
                    + 3:temp_li_uni.index('mlpg') + 4]
                sex = ''.join(sex)

                if unidecode(str(sex)) == 'pkrss':
                    dic['sex'] = 'Male'
                else:
                    dic['sex'] = 'Female'
                ...
                if 'husband_name' in dic.keys():
                    ...
                    s = dic['ID'] + ',' + dic['name'] + ',' + dic['age'
                            ] + ',' + dic['sex'] + ',' + dic['house_num'
                            ] + ',' + 'NONE' + ',' + dic['husband_name']
                    my_list.append(s)
                else:

                    s = dic['ID'] + ',' + dic['name'] + ',' + dic['age'
                            ] + ',' + dic['sex'] + ',' + dic['house_num'
                            ] + ',' + dic['father_name'] + ',' + 'NONE'
                    my_list.append(s)
